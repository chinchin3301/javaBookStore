...
public class AddGenreAdminAction implements Action{
    ...
    public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException, SQLException, ParseException {
        ...
        if(session.getAttribute(USER_ID) == null || session.getAttribute(IS_ADMIN) == null
            ...
            String newGenreTitle = request.getParameter("new_genre_title");
            if(!UChecker.isCheckTitle(newGenreTitle) || (bookGenreDAO.getId(newGenreTitle, localId) != null &&
                    bookGenreDAO.getId(newGenreTitle, localId) != 0L)) {
                response.sendRedirect(ERROR_JSP);
            }else {
                BookGenre bookGenre = new BookGenre();
                bookGenre.setTitle(newGenreTitle);
                bookGenre.setLocalsId(localId);
